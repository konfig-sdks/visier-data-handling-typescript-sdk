...
import { readableStreamToString, VisierDataHandlingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierDataHandlingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierDataHandlingError(e, e.response?.data)
                }
                if (innerError instanceof VisierDataHandlingError) {
                    // Got "VisierDataHandlingError" from the above try block
                    throw innerError;
                }
